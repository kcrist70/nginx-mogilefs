...
objs/nginx:	objs/src/core/nginx.o \
	...
	objs/addon/nginx-mogilefs-module-1.0.4/ngx_http_mogilefs_module.o \
	objs/ngx_modules.o
...
objs/addon/nginx-mogilefs-module-1.0.4/ngx_http_mogilefs_module.o:	$(ADDON_DEPS) \
	/usr/local/src/nginx-mogilefs-module-1.0.4//ngx_http_mogilefs_module.c
	$(CC) -c $(CFLAGS)  $(ALL_INCS) \
		-o objs/addon/nginx-mogilefs-module-1.0.4/ngx_http_mogilefs_module.o \
		/usr/local/src/nginx-mogilefs-module-1.0.4//ngx_http_mogilefs_module.c
...
install:	build 
	test -d '$(DESTDIR)/usr' || mkdir -p '$(DESTDIR)/usr'
	...
	test -d '$(DESTDIR)/var/run/nginx' \
		|| mkdir -p '$(DESTDIR)/var/run/nginx'

	test -d '$(DESTDIR)/var/log/nginx' \
		|| mkdir -p '$(DESTDIR)/var/log/nginx'

	test -d '$(DESTDIR)/usr/html' \
		|| cp -R html '$(DESTDIR)/usr'
